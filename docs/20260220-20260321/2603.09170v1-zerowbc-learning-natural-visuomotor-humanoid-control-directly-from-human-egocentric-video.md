...
tags: ["query:rob-il"]
score: 8.0
evidence: 直接从人类第一视角视频中学习视觉运动类人控制
tldr: ZeroWBC是一个直接从人类第一视角视频中学习人形机器人全身控制的框架。针对现有方法依赖昂贵遥操作数据且动作僵硬的问题，该研究通过微调视觉语言模型（VLM）预测人类动作，并将其重定向至机器人关节，实现了无需遥操作数据的自然交互控制。在Unitree G1机器人上的实验证明了其在动作自然度和通用性方面的优越性，为人形机器人控制提供了高效可扩展的新范式。
selection_source: fresh_fetch
motivation: 传统人形机器人控制依赖昂贵的遥操作数据且动作不自然，难以实现如坐下或踢球等复杂的类人交互行为。
method: 通过微调VLM根据文本指令和第一视角视觉上下文预测人体运动，并利用鲁棒的通用运动跟踪策略将生成的动作重定向至机器人。
result: 在Unitree G1机器人上的实验表明，该方法在动作自然度和任务通用性上均优于基准方法，且无需任何机器人遥操作数据。
conclusion: ZeroWBC成功建立了一套从人类视频直接学习控制的流水线，消除了数据采集成本，为通用人形机器人全身控制提供了高效方案。
---
