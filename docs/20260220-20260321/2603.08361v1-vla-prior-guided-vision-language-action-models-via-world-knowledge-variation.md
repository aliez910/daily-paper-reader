...
tldr: 本研究针对现有视觉-语言-动作（VLA）模型仅关注预测未来绝对状态而忽视变化过程的问题，提出了ΔVLA框架。该框架通过建模相对于当前世界知识先验的变化来指导动作生成。核心贡献包括：利用PWKE提取关键空间与语义先验，通过LWVQ在离散潜空间中量化知识演变，以及设计CV-Atten减少建模干扰。实验证明，ΔVLA在模拟和真实机器人任务中均取得SOTA性能，且具备更高的效率。
selection_source: fresh_fetch
motivation: 现有VLA模型侧重于预测绝对的未来世界状态，而忽视了对指导动作生成的关键变化过程的推理。
method: 提出ΔVLA框架，通过PWKE提取世界知识先验，利用LWVQ量化知识变化，并结合CV-Atten实现解耦的变分建模。
result: 在模拟基准测试和真实机器人任务中均达到了SOTA性能，并显著提升了推理效率。
conclusion: 通过建模世界知识的相对变化而非绝对状态，ΔVLA为视觉-语言-动作模型提供了一种更高效且精准的动作生成范式。
---
