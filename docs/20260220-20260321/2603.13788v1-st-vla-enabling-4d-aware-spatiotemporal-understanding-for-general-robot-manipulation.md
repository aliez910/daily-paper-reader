...
tags: ["query:rob-il"]
score: 10.0
evidence: 具有 4D 感知能力的通用机器人操纵分层 VLA 框架
tldr: 针对现有机器人操控框架缺乏深度感知和时间一致性的问题，本文提出ST-VLA。该框架通过统一的3D-4D表示连接感知与动作，利用ST-VLM生成空间接地且时间连贯的3D轨迹和掩码。配合包含30万个片段的大规模数据集ST-Human，ST-VLA在RLBench和真实场景中显著提升了零样本成功率，增强了复杂3D环境下的鲁棒性。
selection_source: fresh_fetch
motivation: 现有的VLA框架多依赖2D表示，在处理复杂3D场景时缺乏深度感知和时间一致性，限制了操控的鲁棒性。
method: 提出ST-VLA框架，通过ST-VLM模型生成具有4D时空上下文的3D轨迹和空间掩码，并构建了大规模多维标注数据集ST-Human进行训练。
result: "在RLBench和真实世界任务中，ST-VLA将零样本成功率分别提升了44.6%和30.3%，显著优于现有最先进的基准模型。"
conclusion: 将时空推理任务交给具备统一3D-4D表示的视觉语言模型，能大幅提升开放世界机器人操控的泛化能力和鲁棒性。
---
