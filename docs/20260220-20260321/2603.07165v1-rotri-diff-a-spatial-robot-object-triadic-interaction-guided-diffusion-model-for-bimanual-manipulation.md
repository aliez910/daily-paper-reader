...
tags: ["query:rob-il"]
score: 9.0
evidence: 使用扩散模型的双臂操纵模仿学习
tldr: 针对双臂协作机器人任务中存在的碰撞和抓取不稳定问题，本文提出了RoTri-Diff框架。该方法通过显式建模机器人双臂与物体之间的三元空间交互关系（RoTri），利用扩散模型生成具有几何约束的协调轨迹。该框架结合了机器人关键位姿和物体运动，实现了分层扩散过程，显著提升了双臂操作的稳定性和成功率。
selection_source: fresh_fetch
motivation: 现有的双臂操作方法忽视了双臂与物体间的动态几何关系，导致任务执行中容易发生碰撞且抓取不够稳定。
method: 提出RoTri表示法来编码双臂与物体的相对6D位姿，并将其集成到分层扩散模型中以生成受几何约束的动作轨迹。
result: "在RLBench2的11个任务中比现有最优方法提升了10.2%的性能，并在4个真实世界挑战性任务中表现稳定。"
conclusion: 显式建模双臂与物体的三元交互关系能有效增强双臂协作的协调性与鲁棒性。
---
