...
title_zh: ManipulationNet：一种用于基准测试真实世界机器人操作的基础设施，包含物理技能挑战与具身多模态推理
...
tags: ["query:rob-il"]
score: 9.0
evidence: 用于基准测试真实世界机器人操纵物理技能挑战的基础设施
...
## 摘要
灵巧操作使机器人能够有目的地改变物理世界，使其从非结构化环境中的被动观察者转变为主动智能体。这种能力是物理人工智能的基石。尽管在硬件、感知、控制和学习方面取得了数十年的进展，但由于缺乏广泛采用的标准基准，通用操作系统的进展仍然是碎片化的。核心挑战在于如何协调真实世界的多变性与严谨科学评估所需的重复性和真实性。为了解决这一问题，我们推出了 ManipulationNet，这是一个托管真实世界机器人操作基准任务的全球基础设施。ManipulationNet 通过标准化的硬件套件提供可重复的任务设置，并通过统一的软件客户端实现分布式性能评估，该客户端可提供实时任务指令并收集基准测试结果。作为一个持久且可扩展的基础设施，ManipulationNet 将基准任务组织为两个互补的赛道：1) 物理技能赛道，评估底层物理交互技能；2) 具身推理赛道，测试高层推理和多模态落地能力。这种设计促进了真实世界能力与技能互联网络的系统性增长，为通向通用机器人操作铺平了道路。通过在大规模真实世界中实现可比的操作研究，该基础设施为衡量长期科学进展和识别具备实际部署能力的技术奠定了可持续的基础。
