...
tags: ["query:rob-il"]
score: 10.0
evidence: 用于细粒度操纵的运动学感知视觉-语言-动作（VLA）模型
...
method: 提出一种双层动作表示方法，利用推理Token作为中间变量，将语言指令显式解耦为目标层级和运动学层级的动作。
result: 在LIBERO和Realman-75机器人上的实验证明，KineVLA在运动学敏感的任务中表现优于强基准模型，实现了更精准的操控。
