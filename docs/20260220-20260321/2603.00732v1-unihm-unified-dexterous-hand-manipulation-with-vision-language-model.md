...
method: 开发了统一灵巧手分词器和视觉语言动作模型，并引入物理引导的动态精炼模块以确保操作序列的物理可行性。
result: 在多个数据集和真实世界评估中，UniHM在处理已知和未知物体及轨迹时均取得了最先进的性能表现。
conclusion: 该研究证明了结合视觉语言模型与人类交互数据可以实现高度通用且物理真实的灵巧手操作规划。
---
