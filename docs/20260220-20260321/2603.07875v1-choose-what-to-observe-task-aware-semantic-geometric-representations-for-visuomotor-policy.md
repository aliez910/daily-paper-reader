...
tags: ["query:rob-il"]
score: 9.0
evidence: 具有任务感知语义几何表示的视觉运动策略
tldr: 针对视觉运动策略易受背景和颜色等干扰因素影响而过拟合的问题，本文提出一种任务感知的观测接口。该方法利用SAM3分割任务相关实体，并结合语义着色与单目深度估计构建统一的语义-几何表示。实验证明，该接口在不修改或微调策略模型的情况下，显著提升了策略在分布外视觉变化下的鲁棒性，具有极强的泛化能力。
...
result: 在多个仿真和真实机器人基准测试中，该方法在保持分布内性能的同时，大幅增强了策略对视觉偏移的鲁棒性。
conclusion: 通过将原始视觉输入规范化为任务相关的语义-几何表示，可以有效提升视觉运动策略的泛化性和鲁棒性。
---
