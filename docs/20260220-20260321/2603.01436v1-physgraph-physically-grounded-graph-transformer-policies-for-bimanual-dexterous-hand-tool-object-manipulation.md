...
tldr: PhysGraph 针对双手机巧手操作工具的复杂任务，提出了一种基于物理基础的图 Transformer 策略。它将双手系统建模为运动学图，通过逐链接标记化保留细粒度状态，并引入物理偏置生成器将运动学、接触和几何等先验注入注意力机制。该方法在提高操作精度和成功率的同时，显著减少了参数量，并展现出强大的零样本迁移能力和跨机器人手的通用性。
selection_source: fresh_fetch
motivation: 现有方法将系统状态简化为单一向量，忽略了机巧手固有的结构和拓扑信息，难以处理复杂的接触动力学。
method: 将双手系统表示为运动学图，采用逐链接标记化，并利用包含空间距离、接触状态和几何邻近性的物理偏置指导注意力机制。
result: "在操作精度和成功率上显著优于 ManipTrans，参数量仅为其 51%，且支持对未知几何体的零样本迁移。"
conclusion: PhysGraph 证明了显式建模物理结构和拓扑信息对于提升双手机巧手操作任务的性能和泛化性至关重要。
---
