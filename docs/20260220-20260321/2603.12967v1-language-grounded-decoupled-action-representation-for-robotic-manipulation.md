...
tags: ["query:rob-il"]
score: 8.0
evidence: 从感知到动作原语的语言对齐映射
tldr: 针对机器人操作中高层视觉语言理解与底层动作控制之间的异构性挑战，本文提出LaDA框架。该框架利用自然语言作为语义桥梁，将动作解耦为平移、旋转和夹爪控制三个可解释基元。通过语义引导的软标签对比学习和自适应加权策略，LaDA实现了跨任务的动作对齐，显著增强了机器人在处理新任务或语义相关任务时的泛化能力和动作一致性。
selection_source: fresh_fetch
motivation: 旨在解决机器人操作中高层语义理解与底层动作控制不匹配，导致在处理新任务时泛化性差的问题。
method: 提出LaDA框架，将动作解耦为平移、旋转和夹爪控制三个语义基元，并采用语义引导的对比学习与自适应加权策略。
result: 在LIBERO、MimicGen仿真环境及真实世界测试中，LaDA在任务成功率和对未知任务的泛化表现上均优于现有方法。
conclusion: 通过引入细粒度的解耦动作表示和语义对齐机制，LaDA为提升机器人操作的鲁棒性和跨任务迁移能力提供了有效方案。
---
