...
date: 2026-03-10
pdf: "https://arxiv.org/pdf/2603.09565v1"
tags: ["query:rob-il"]
score: 9.0
evidence: 用于精密装配的视触觉融合模仿学习策略
tldr: "针对精密装配中视觉遮挡导致的“最后一毫米”难题，本文提出ReTac-ACT模仿学习策略。该方法通过双向交叉注意力实现视触觉特征互补，利用本体感受门控网络在遮挡时动态提升触觉权重，并引入触觉重建任务以强化接触信息学习。实验表明，该方法在NIST基准测试中表现优异，尤其在0.1mm工业级间隙下仍能保持80%的成功率，显著提升了装配精度。"
selection_source: fresh_fetch
motivation: 解决精密装配过程中因末端执行器遮挡导致视觉反馈失效，从而难以实现亚毫米级精度修正的挑战。
method: 提出一种集成双向交叉注意力、本体感受门控机制和触觉重建目标的视触觉融合Transformer架构。
result: "在NIST装配任务中达到90%成功率，并在0.1mm极小间隙下显著优于纯视觉和通用基准方法。"
conclusion: 研究验证了动态视触觉融合与触觉特征重建在处理高精度、多遮挡装配任务中的必要性和有效性。
---

## 摘要
精密装配需要在接触密集的“最后一毫米”区域进行亚毫米级的修正，而在这些区域，由于末端执行器和工件的遮挡，视觉反馈往往会失效。我们提出了 ReTac-ACT（重构增强型触觉 ACT），这是一种视触觉模仿学习策略，通过三种协同机制解决这一挑战：(i) 双向交叉注意力机制，在融合前实现视触觉特征的相互增强；(ii) 以本体感受为条件的状态门控网络，在发生视觉遮挡时动态提高对触觉的依赖；(iii) 触觉重构目标，强制学习与操作相关的接触信息，而非通用的视觉纹理。在标准化的 NIST 装配任务板 M1 基准测试中，ReTac-ACT 实现了 90% 的轴孔装配成功率，显著优于纯视觉和通用基准方法，并在工业级 0.1mm 间隙下保持了 80% 的成功率。消融实验验证了每个架构组件都是不可或缺的。ReTac-ACT 代码库以及涵盖各种间隙水平、包含视觉和触觉特征的视触觉演示数据集将公开发布，以支持可重复性研究。
