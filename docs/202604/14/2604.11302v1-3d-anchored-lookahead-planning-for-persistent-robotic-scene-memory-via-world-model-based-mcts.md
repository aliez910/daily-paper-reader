...
tags: ["query:rob-il"]
score: 8.0
evidence: 基于世界模型的复杂机器人操纵规划
tldr: 本研究提出3D-ALP，一种结合蒙特卡洛树搜索（MCTS）与3D一致性世界模型的机器人操纵推理引擎。该方法通过维护持久的相机到世界锚点，解决了反应式策略在物体遮挡时无法规划的问题，实现了对不可见物体的精确重规划。实验表明，3D-ALP在需要空间记忆的复杂任务中表现优异，并解决了UCT-MCTS在连续操纵中的结构性失效。
selection_source: fresh_fetch
motivation: 传统的反应式策略缺乏空间记忆，导致机器人在物体被遮挡或移出视野时无法进行有效的长程规划。
method: 结合MCTS与3D一致性世界模型，利用持久的3D锚点建立跨时间步的场景记忆，并优化了UCT算法以适应连续操纵空间。
result: 在五步顺序抓取任务中，3D-ALP将记忆相关步骤的成功率从0.006提升至0.650，最后一步成功率高达0.822。
conclusion: 3D锚定的前瞻规划显著增强了机器人的空间记忆和推理能力，是实现复杂机器人操纵的关键。
---

## 摘要
我们提出了 3D 锚定前瞻规划（3D-ALP），这是一种用于机器人操作的系统 2 推理引擎，它将蒙特卡洛树搜索（MCTS）与作为展开预言机（rollout oracle）的 3D 一致性世界模型相结合。与仅根据当前相机帧评估动作的反应式策略不同，3D-ALP 维护一个持久的相机到世界坐标系（c2w）锚点，该锚点在遮挡情况下依然有效，从而能够对不再直接可见的物体位置进行准确的重新规划。在需要空间记忆的 5 步序列到达任务（实验 E3）中，3D-ALP 在需要记忆的步骤上实现了 0.650 ± 0.109 的成功率，而贪婪反应式基准仅为 0.006 ± 0.008（Δ=+0.645）；在第 5 步的成功率上，3D-ALP 达到了 0.822，而贪婪基准为 0.000。消融实验（30 个回合，3 个随机种子）表明，树搜索空间记忆是主要的性能驱动因素（贡献了 +0.533，占增益的 82%），而更深的前瞻规划也带来了额外收益（贡献了 +0.111，占 17%）。我们还识别并解决了将 UCT-MCTS（应用于树的上置信界算法）应用于连续机器人操作时的四种结构性失效模式。
