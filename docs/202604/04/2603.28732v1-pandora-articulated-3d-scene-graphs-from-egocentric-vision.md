...
motivation: 传统机器人建图受限于机器人自身的物理局限性，难以获取如抽屉内部等被遮挡或需交互的区域信息。
method: 利用人类探索环境时的第一视角视频数据，通过启发式算法提取物体关节模型，并将其整合进3D场景图中。
result: 提取的关节模型质量可媲美现有先进方法，且生成的场景图能显著增强机器人执行复杂移动操作任务的能力。
conclusion: 通过人类视角数据构建关节化3D场景图，是弥补机器人感知盲区并提升其环境交互能力的有效途径。
---
