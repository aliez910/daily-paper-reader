...
tags: ["query:rob-il"]
score: 8.0
evidence: 执行状态监控和抓取的闭环智能体循环
tldr: 针对机器人语言引导抓取中常见的单次执行且缺乏反馈的问题，本文提出了一种受数字智能体启发的物理智能体循环框架。该框架通过引入名为Watchdog的执行监控层，将遥测数据转化为离散状态标签，并结合确定性策略实现对抓取失败的自动重试或用户求助。实验证明，该方法在不改变底层模型的情况下，显著提升了机器人在复杂环境下的鲁棒性和可解释性。
