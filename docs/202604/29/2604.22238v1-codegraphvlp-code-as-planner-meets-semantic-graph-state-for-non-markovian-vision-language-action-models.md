...
tags: ["query:rob-il"]
score: 9.5
evidence: 通过语义图和基于代码的规划增强了用于通用机器人操纵的视觉-语言-动作（VLA）模型。
tldr: CodeGraphVLP通过将语义图状态与VLA模型集成，提升了长程机器人操纵任务的性能。
source: arxiv
...
motivation: 通过语义图和基于代码的规划增强了用于通用机器人操纵的视觉-语言-动作（VLA）模型。
