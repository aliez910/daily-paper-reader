...
tldr: FEEL是首个将定制压阻手套采集的力测量数据与第一视角视频相结合的大规模数据集，包含约300万帧自然厨房操作场景。该研究强调力是理解物理交互的关键原语，通过力同步数据实现了接触理解和自监督动作表示学习。实验表明，FEEL在时间接触分割上达到SOTA水平，并在无需手动标注的情况下显著提升了在EPIC-Kitchens等多个主流动作理解任务中的迁移性能。
selection_source: fresh_fetch
motivation: 物理力是驱动交互的核心因素，但现有视频数据集缺乏大规模且同步的力信息来深入理解物理动作。
method: 利用定制压阻手套采集大规模力-视频同步数据，并以力预测作为自监督预训练目标来优化视频表征。
result: 在时间接触分割任务中取得领先结果，并在多个动作理解基准测试中展现出优于传统数据集的迁移学习效果。
conclusion: FEEL数据集证明了力信息作为物理交互原语在提升计算机视觉模型动作理解能力方面的关键作用。
---
