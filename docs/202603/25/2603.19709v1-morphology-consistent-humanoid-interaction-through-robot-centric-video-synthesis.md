...
## 摘要
为类人机器人配备多样的交互技能通常需要大量的策略训练或显式的人到机器人动作重定向。然而，基于学习的策略面临着极高的轨迹数据采集成本。同时，重定向依赖于以人为中心的姿态估计（例如 SMPL），这引入了形态差异。当映射到机器人时，骨架比例的不匹配会导致严重的空间对齐偏差，从而损害交互的成功率。在这项工作中，我们提出了 Dream2Act，这是一个通过生成式视频合成实现零样本交互的以机器人为中心的框架。给定机器人和目标物体的第三人称图像，我们的框架利用视频生成模型来构思机器人完成任务的形态一致运动。我们采用高保真姿态提取系统，从这些合成的“梦境”中恢复物理上可行的机器人原生关节轨迹，随后通过通用全身控制器执行。Dream2Act 严格在机器人原生坐标空间内运行，避免了重定向误差，并消除了特定任务的策略训练。我们在宇树 G1 机器人上针对四种全身移动交互任务（踢球、坐沙发、打沙袋和抱箱子）评估了 Dream2Act。Dream2Act 实现了 37.5% 的总成功率，而传统重定向方法的成功率为 0%。由于形态差异（且误差在移动过程中会累积），重定向无法建立正确的物理接触，而 Dream2Act 保持了机器人一致的空间对齐，从而实现了可靠的接触形成并显著提高了任务完成率。
