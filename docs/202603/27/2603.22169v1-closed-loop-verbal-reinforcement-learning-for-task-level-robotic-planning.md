...
tldr: 本研究针对移动机器人在不确定环境下的任务规划，提出了一种闭环言语强化学习（VRL）框架。该框架通过大语言模型生成可执行的行为树，并利用视觉语言模型观察执行过程并提供自然语言反馈，从而实现策略的迭代优化。与传统强化学习不同，VRL在符号规划层面进行更新，无需梯度优化，显著提升了策略演化的可解释性。实验证明，该方法能有效应对执行失败，并在真实机器人系统上实现可靠部署。
selection_source: fresh_fetch
motivation: 旨在解决移动机器人在不确定环境下执行复杂任务时，传统规划方法缺乏闭环自适应能力和决策透明度的问题。
method: 提出一种基于言语强化学习的闭环架构，利用LLM作为执行者优化行为树，并结合VLM提供的自然语言反馈进行非梯度的符号级策略更新。
result: 在真实机器人的多阶段操作与导航实验中，该框架展示了卓越的故障自适应能力和人类可理解的策略进化过程。
conclusion: 该研究验证了通过自然语言反馈在符号层面进行闭环学习的可行性，为构建可解释且鲁棒的机器人规划系统提供了新路径。
---
