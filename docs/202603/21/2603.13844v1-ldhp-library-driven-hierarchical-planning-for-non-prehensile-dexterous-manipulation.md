...
motivation: 旨在解决非抓取式灵巧操作中现有规划方法物理不可行以及学习方法数据依赖性强、泛化能力差的问题。
method: 提出一种分层规划框架，利用MoveObject和AdjustGrasp原语分别规划物体路径与抓取序列，并结合物理约束进行可行性验证。
result: 在真实机器人的零机动性提升和插槽任务中，该方法展现了稳定的执行效果以及对物体形状和环境变化的鲁棒性。
conclusion: LDHP通过解耦物体运动与抓取可行性，提供了一个无需重新设计即可跨任务和几何变体迁移的通用操作流水线。
---
