...
title_zh: 基于具身视觉语言规划的事件驱动主动辅助操纵
...
## 摘要
协作操纵中的辅助通常由用户指令发起，这使得高层推理呈现为请求驱动模式。然而，在流畅的人类团队合作中，合作伙伴通常根据观察到的动作结果推断下一步有益的操作，而不是等待指令。受此启发，我们提出从请求驱动辅助向事件驱动主动辅助的转变，其中机器人动作由人机交互引起的工作空间状态转换触发，而非由用户提供的任务指令触发。为此，我们提出了一个事件驱动框架，该框架利用事件监测器跟踪交互进度，并在事件完成后提取表征状态转换的稳定前后快照。基于这些稳定快照，规划器分析隐含的状态转换以推断任务级目标，并决定是否介入；如果介入，则生成一系列辅助动作。为了使输出可执行且可验证，我们将动作限制在一组动作原语中，并通过整数 ID 引用目标对象。我们在真实的桌面数字积木协作任务上评估了该框架，结果表明，明确的前后状态变化证据提高了在可解场景中的主动完成率，并在不可解场景中实现了适当的等待。
