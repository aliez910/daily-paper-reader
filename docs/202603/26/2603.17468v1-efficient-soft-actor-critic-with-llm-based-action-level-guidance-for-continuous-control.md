...
tldr: 针对强化学习在广阔状态动作空间中探索效率低的问题，本文提出GuidedSAC算法。该方法利用大语言模型（LLM）作为智能监督者，通过分析轨迹信息提供动作级引导，辅助Soft Actor-Critic（SAC）进行针对性探索。理论证明其在保持SAC收敛性的同时提升了收敛速度，实验表明其在MuJoCo等任务中的样本效率和性能显著优于SAC及多种主流探索增强变体。
selection_source: fresh_fetch
motivation: 旨在解决强化学习在复杂连续控制任务中因状态动作空间巨大而导致的探索效率低下和样本利用率低的问题。
method: 提出GuidedSAC算法，利用LLM作为监督者分析状态和视觉回放，为SAC提供动作级的干预引导以实现针对性探索。
result: 在离散任务和MuJoCo连续控制基准测试中，GuidedSAC的样本效率和最终性能均优于标准SAC及RND、ICM、E3B等先进探索算法。
conclusion: GuidedSAC通过引入LLM的动作级引导，有效增强了复杂环境下的探索能力，并提供了提升收敛速度的理论保证。
---
