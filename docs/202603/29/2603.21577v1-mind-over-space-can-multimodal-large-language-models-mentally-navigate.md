...
tags: ["query:rob-il"]
score: 6.0
evidence: 用于空间推理和路径规划的多模态大模型
tldr: 针对多模态大模型（MLLM）在长程空间推理和心理导航能力的缺失，本文提出了Video2Mental评测基准，要求模型从第一视角视频构建认知地图并规划路径。同时，开发了NavMind模型，通过将显式认知地图作为可学习的中间表示，并采用难度分层的渐进式监督微调，显著提升了模型在复杂环境下的导航规划能力，弥补了感知与结构化规划之间的鸿沟。
selection_source: fresh_fetch
motivation: 现有具身智能模型多依赖即时观察的反应式规划，缺乏生物智能中构建空间表征并进行心理模拟的“心理导航”能力。
method: 引入Video2Mental基准评估心理导航能力，并提出NavMind模型，利用细粒度认知地图作为中间推理表示进行渐进式微调。
result: 实验表明前沿MLLM在长程空间推理上表现不佳，而NavMind在导航规划准确性上显著超越了现有商业和空间大模型。
conclusion: 将显式认知地图内化为中间推理步骤是提升大模型实现类人心理导航和复杂空间推理能力的关键。
---
