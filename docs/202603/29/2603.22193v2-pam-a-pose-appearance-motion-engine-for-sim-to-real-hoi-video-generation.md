...
motivation: 现有的手物交互生成研究在姿态合成、单图生成和视频生成三个方向上相互孤立，缺乏一个能够统一处理姿态、外观与运动的连贯框架。
method: 提出了一种名为PAM的姿态-外观-运动引擎，通过结合深度图、分割掩码和关键点等多重条件，实现可控的高分辨率HOI视频生成。
result: 在DexYCB数据集上，PAM将FVD指标从38.83显著降至29.13，且其生成的合成数据能有效辅助下游手部姿态估计模型达到全量真实数据的性能。
conclusion: PAM通过统一多维输入条件，为从模拟环境到真实场景的HOI视频生成提供了一个高效且可扩展的解决方案。
---
